...
public class UI {
    ...
    public void init(){
        ...
        frame.add(chessboardService.getChessboardComponent());//把五子棋盘加入到frame
        ...
        chessboardService.getChessboardComponent().addMouseListener(new MouseAdapter(){
    ...
    public void replayOptionInit(){
        replayOption = new AbstractAction("重玩一盘", new ImageIcon("image/replay.png")){
            public void actionPerformed(ActionEvent e){
                chessboardService.init();//界面方面：初始化重来
    ...
    public void AIFirstOptionInit(){
        AIFirstOption = new AbstractAction("机器先手", new ImageIcon("image/robot.png")){
            public void actionPerformed(ActionEvent e){
                ...
                if(chessboardService.isEmpty()){
                    ChessEntity.FIRST = -1;
                    //机器先手，则先在中间位置下一个棋子
                    chessboardService.addChessman(new LocationEntity(7,7,-1));
        ...
        undoOption = new AbstractAction("悔棋", new ImageIcon("image/undo.png")) {
            public void actionPerformed(ActionEvent e) {
                ...
                if (chessService.undoMove()) { // 若棋局逻辑能悔棋
                    chessboardService.undoMove(); // 在界面上也撤销最新的一步棋
                } else {
